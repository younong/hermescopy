...
def create_app(
    ...
    *,
    ...
    app.state.owner_worker_workspace_root = runtime_paths.workspace_root
    ...
    async def _reject_external_owner_selectors(request: Request, call_next):
        ...
        if path.startswith("/api/plugins/"):
            ...
            if len(parts) >= 4 and parts[3]:
                from hermes_cli.dashboard_plugins import plugin_api_enabled

                plugin = next(
                    (
                        candidate
                        for candidate in app.state.owner_worker_dashboard_plugins
                        if candidate.get("name") == parts[3]
                    ),
                    None,
                )
                if plugin is None or not plugin_api_enabled(plugin):
                    return JSONResponse(
                        status_code=404,
    ...
    class _CronGatewayTransport:
        # In-process marker checked by the gateway before it trusts an
        # employee identity on a ``source="cron"`` session.create — remote
        # transports are server-created and can never carry it.
        cron_internal = True
    ...
    def _run_agent_cron_job(job: dict, fire_id: str) -> dict | None:
        ...
        try:
            ...
            employee_id = str(job.get("employee_id") or "").strip()
            create_params: dict = {
                "source": "cron",
                "title": str(job.get("name") or "Scheduled job"),
            ...
            if employee_id:
                # Employee jobs run under the employee's current policy, which
                # the gateway resolves; runtime overrides (model/provider/cwd)
                # are mutually exclusive with an employee identity.
                create_params["employee_id"] = employee_id
            else:
                create_params["cwd"] = _cron_job_workdir(job)
                if job.get("model"):
                    create_params["model"] = job["model"]
                if job.get("provider"):
                    create_params["provider"] = job["provider"]
            created = _gateway_call(transport, "session.create", create_params)
    ...
    def get_dashboard_plugins(_: None = Depends(_require_owner_token)) -> list[dict[str, Any]]:
        ...
        return active_dashboard_plugin_payload(
            app.state.owner_worker_dashboard_plugins,
        )
    ...
    # Mount only APIs whose validated manifests explicitly target Owner Workers.
    # Discovery runs after HERMES_HOME admission, preserving per-owner user plugin
    # roots while bundled APIs remain shared and project Python stays untrusted.
    from hermes_cli.dashboard_plugins import (
        discover_dashboard_plugins,
        mount_dashboard_plugin_apis,
    )

    owner_worker_plugins = discover_dashboard_plugins()
    app.state.owner_worker_dashboard_plugins = owner_worker_plugins
    mount_dashboard_plugin_apis(
        app,
        owner_worker_plugins,
        runtime_target="owner-worker",
        owner_dependency=_require_owner_token,
    )
