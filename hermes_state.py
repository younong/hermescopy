...
CREATE TABLE IF NOT EXISTS sessions (
    ...
    context_usage_route_key TEXT,
    context_usage_calibration_ratio REAL,
...
class SessionDB(SessionQueryMixin):
    """
    ...
    def __init__(self, db_path: Path = None, read_only: bool = False):
        ...
        try:
            if read_only:
                ...
                self._fts_enabled = self._read_table_exists("messages_fts")
                self._trigram_available = self._read_table_exists(
    ...
    def _read_table_exists(self, table: str) -> bool:
    ...
    def record_context_usage_calibration(
        ...
        """Persist conservative rough-to-authoritative prompt calibration."""
        if not session_id or not route_key:
            return

        def _do(conn):
            conn.execute(
                "UPDATE sessions SET context_usage_route_key = ?, "
                "context_usage_calibration_ratio = ?, "
                "context_usage_calibration_samples = ? WHERE id = ?",
                (route_key, max(1.0, float(ratio)), max(0, int(samples)), session_id),
            )

        self._execute_write(_do)

    def get_context_usage_calibration(
        self,
        session_id: str,
        route_key: str,
    ) -> Optional[Dict[str, Any]]:
        """Return calibration only when it belongs to the current model route."""
        if not session_id or not route_key:
            return None
        with self._lock:
            row = self._conn.execute(
                "SELECT context_usage_route_key, context_usage_calibration_ratio, "
                "context_usage_calibration_samples FROM sessions WHERE id = ?",
                (session_id,),
            ).fetchone()
        if row is None or row["context_usage_route_key"] != route_key:
            return None
        ratio = row["context_usage_calibration_ratio"]
        if ratio is None:
            return None
        return {
            "ratio": float(ratio),
            "samples": int(row["context_usage_calibration_samples"] or 0),
        }
