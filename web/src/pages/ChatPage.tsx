...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  useEffect(() => {
    // The terminal host stays mounted behind display:none while users visit
    // other dashboard routes. It must not create or retarget PTY/event sockets
    // in response to Chat GUI's shared ?resume= parameter while inactive.
    if (!isActive) return;
    const host = hostRef.current;
    if (!host) return;
    ...
    // fast, but its texture atlas can get out of sync with large scrollback
    // replays from resumed historical sessions: the bottom is visible, older
    // viewport ranges can appear blank, and a browser resize fixes it by
    // forcing a full renderer rebuild. The default renderer is stable for
    // scrollback-heavy history browsing, which is the dashboard chat's common
    // path.
    ...
    ws.onclose = (ev) => {
      ...
      if (ev.code === 4409) {
        setBanner(
          ev.reason
            ? `Chat already open: ${ev.reason}.`
            : "Chat is already open in this browser.",
        );
        return;
      }
      if (ev.code === 1011) {
