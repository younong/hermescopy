...
def test_authority_worker_start_failure_diagnostics_are_allowlisted(tmp_path, monkeypatch):
    control_home = tmp_path / "control-plane"
    control_home.mkdir()
    monkeypatch.setenv("HERMES_CONTROL_HOME", str(control_home))
    ...
    audit_authority(
        ...
        correlation_id="a" * 32,
        reason=AuthorityAuditReason.GENERATION_START_FAILED,
        audience_class="none",
        worker_generation=7,
        recovery_generation=2,
        failure_stage="health_verify",
        failure_type="OwnerWorkerStartupError",
        worker_id="b" * 32,
        process_id=1234,
        exit_code=-15,
        socket_exists=True,
    ...
    entry = json.loads((control_home / "logs" / "authority.log").read_text())
    assert entry["failure_stage"] == "health_verify"
    assert entry["failure_type"] == "OwnerWorkerStartupError"
    assert entry["worker_id"] == "b" * 32
    assert entry["process_id"] == 1234
    assert entry["exit_code"] == -15
    assert entry["socket_exists"] is True


@pytest.mark.parametrize(
    ("event", "reason", "field", "value"),
    [
        (
            AuthorityAuditEvent.TICKET_REJECTED,
...
def test_authority_worker_start_failure_diagnostics_reject_invalid_values(
    profile_home, event, reason, field, value,
):
    kwargs = {field: value}
...
def test_authority_worker_start_failure_diagnostics_reject_non_boolean_socket_state(
    profile_home,
):
    with pytest.raises(ValueError, match="socket_exists"):
        audit_authority(
            AuthorityAuditEvent.WORKER_GENERATION,
            correlation_id="d" * 32,
            reason=AuthorityAuditReason.GENERATION_START_FAILED,
            failure_stage="socket_wait",
