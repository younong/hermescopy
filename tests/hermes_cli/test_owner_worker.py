...
def test_owner_worker_skill_sync_cache_skips_fresh_owner(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        supervisor_module,
        "_sync_owner_skills",
        lambda owner_home, bundled_snapshot=None: syncs.append(owner_home) or {"copied": ["test"]},
    )
...
def test_owner_worker_skill_sync_cache_invalidates_source_revision(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        supervisor_module,
        "_sync_owner_skills",
        lambda owner_home, bundled_snapshot=None: syncs.append(owner_home) or {"copied": []},
    )
...
def test_owner_worker_skill_sync_cache_does_not_stamp_failed_sync(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(supervisor_module, "_sync_owner_skills", lambda *_args, **_kwargs: None)
...
def test_worker_analytics_and_model_info_routes_require_owner_token(tmp_path, monkeypatch):
    ...
    assert client.get("/api/model/options").status_code == 401
...
def test_worker_model_options_returns_owner_local_picker_and_rejects_profile(
    ...
    control_home = tmp_path / "control"
    monkeypatch.setenv("HERMES_HOME", str(owner_home))
    monkeypatch.setenv("HERMES_OWNER_KEY", "ok1_worker_model_options")
    monkeypatch.setenv("HERMES_CONTROL_HOME", str(control_home))
    ...
    def _build_models_payload(context, **kwargs):
        observed["context"] = context
        observed["kwargs"] = kwargs
        return {"providers": [], "model": "owner-model", "provider": "owner-provider"}
    ...
    app = create_app("ok1_worker_model_options", owner_home)
    client = TestClient(app)
    ...
    assert observed["home"] == owner_home.resolve()
    assert observed["context"] == "owner-context"
    assert observed["kwargs"]["include_unconfigured"] is True
