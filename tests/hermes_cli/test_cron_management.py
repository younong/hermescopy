...
def test_employee_job_create_update_and_mutual_exclusion(tmp_path):
    import pytest

    from hermes_cli.cron_management import create_job, get_job, update_job

    home = tmp_path / "profile"
    created = create_job(
        home,
        {"prompt": "daily summary", "schedule": "every 1h", "employee_id": "emp-1"},
    )
    assert created["employee_id"] == "emp-1"
    assert created["provider_snapshot"] is None
    assert created["model_snapshot"] is None

    with pytest.raises(ValueError, match="employee cron jobs cannot set: model"):
        create_job(
            home,
            {
    ...
    # Updating an existing employee job with a conflicting field fails closed
    # against the merged job, not just the payload.
    with pytest.raises(ValueError, match="employee cron jobs cannot set"):
        update_job(home, created["id"], {"skills": ["weekly-report"]})

    # Clearing the employee restores the manual path (and its drift snapshots).
    updated = update_job(home, created["id"], {"employee_id": None, "skills": []})
    assert updated is not None
    assert updated["employee_id"] is None
    assert get_job(home, created["id"])["employee_id"] is None
