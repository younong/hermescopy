...
class TestRollingSummaryPreparation:
    ...
    def test_each_rolling_chunk_gets_a_fresh_request_deadline(self, compressor):
        ...
        assert seen_deadlines == [700.0, 850.0]

    def test_rolling_chunk_progress_reports_real_counts(self, compressor):
        chunks = [
            [{"role": "user", "content": "one"}],
            [{"role": "assistant", "content": "two"}],
        ]
...
class TestCustomCodexResponsesThresholdCap:
    ...
    def test_non_custom_or_non_codex_routes_are_unchanged(self):
        ...
        assert direct.threshold_tokens == 260_000
        assert chat.threshold_tokens == 260_000
    ...
    def test_update_model_recalculates_route_cap(self):
        ...
        assert c.threshold_tokens == 260_000
...
class TestSummaryFallbackToMainModel:
    ...
    def test_model_not_found_404_falls_back_to_main_and_succeeds(self):
        ...
        first_deadline = mock_call.call_args_list[0].kwargs["deadline_monotonic"]
        assert mock_call.call_args_list[1].kwargs["deadline_monotonic"] == first_deadline
...
class TestSummaryTargetRatio:
    ...
    def test_tail_budget_scales_with_context(self):
        ...
        # 200K * 0.65 threshold * 0.40 ratio = 52K
        assert c.tail_token_budget == 52_000

        with patch("agent.context_compressor.get_model_context_length", return_value=1_000_000):
            c = ContextCompressor(model="test", quiet_mode=True, summary_target_ratio=0.40)
        # 1M * 0.65 threshold * 0.40 ratio = 260K
        assert c.tail_token_budget == 260_000
    ...
    def test_default_threshold_is_65_percent(self):
        """Default compression threshold should be 65%, with a 64K floor."""
        with patch("agent.context_compressor.get_model_context_length", return_value=100_000):
            c = ContextCompressor(model="test", quiet_mode=True)
        assert c.threshold_percent == 0.65
        assert c.threshold_tokens == 65_000

    def test_default_threshold_exceeds_floor_for_large_context(self):
        """On large-context models the 65% percentage is used directly."""
        with patch("agent.context_compressor.get_model_context_length", return_value=200_000):
            c = ContextCompressor(model="test", quiet_mode=True)
        assert c.threshold_tokens == 130_000
