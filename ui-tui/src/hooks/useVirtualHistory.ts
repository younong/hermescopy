...
const DASHBOARD_HISTORY_MODE = process.env.HERMES_TUI_DASHBOARD === '1'
const MAX_MOUNTED = DASHBOARD_HISTORY_MODE ? 1000 : 120
const COLD_START = DASHBOARD_HISTORY_MODE ? 120 : 30
...
export function useVirtualHistory(
  ...
  if (DASHBOARD_HISTORY_MODE) {
    // Browser dashboard history browsing favors correctness over incremental
    // virtualization. Mount the full transcript so scroll targets never land
    // in spacer-only gaps while historical sessions are being inspected.
    start = 0
    end = n
  } else if (end - start > maxMounted) {
  ...
  if (!DASHBOARD_HISTORY_MODE && !frozenRange && prevRange.current && vp > 0) {
  ...
  let effStart = DASHBOARD_HISTORY_MODE ? start : start < dStart ? dStart : start
  let effEnd = DASHBOARD_HISTORY_MODE ? end : end > dEnd ? dEnd : end
  ...
  if (!DASHBOARD_HISTORY_MODE && effEnd - effStart > maxMounted && vp > 0) {
