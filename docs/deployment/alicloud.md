...
9. 服务健康检查失败时恢复部署前的 current symlink、runner、systemd units、sandbox policy 和 seccomp artifact，再重启旧版本；成功后再清理旧 release。
...
`tool-public` 与 `protected-target` 继续在 spawn 前明确拒绝：`authenticated network egress is not configured`。不要通过关闭 `--unshare-net` 或回退到进程全局 tool registry 来恢复联网工具；后续必须使用独立 authenticated egress broker。
...
APIYI smoke test 不是发布脚本必跑步骤；需要真实调用模型时再单独执行。发布脚本会做 host sandbox preflight 和 systemd 服务状态检查；生产验收还应使用真实 authenticated 用户验证本地文件/terminal、跨 owner 隔离，以及联网工具的预期拒绝。
