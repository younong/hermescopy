...
Node.js/npm 只要求在本机可用。部署脚本会在本机执行根目录 `npm install --prefer-offline --no-audit` 并构建 `web`、`ui-tui`。
...
3. 把源码 + 构建产物打包上传到 `/opt/hermes/releases/<tag>`。
4. 在服务器上按 `uv.lock` hash 判断是否需要初始化/更新 `/opt/hermes/shared/venv`。
5. 切换 `/opt/hermes/current`。
6. 写入/更新 systemd unit。
7. 重启 `hermes-gateway` 和 `hermes-dashboard`。
