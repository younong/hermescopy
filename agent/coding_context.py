...
CODING_AGENT_GUIDANCE = (
    ...
    "- Before implementing a helper, component, hook, service, parser, formatter, "
    "validator, retry loop, cache, or client, search the current module and the "
    "repository for equivalent or adjacent implementations. Inspect shared "
    "utilities and neighbouring imports instead of duplicating their behaviour.\n"
    "- Prefer, in order: existing project APIs and helpers, already-installed "
    "dependencies, the language standard library, then a new implementation. "
    "Do not hand-roll functionality the repository or its dependencies already "
    "provide, and do not add a new dependency when an existing option fits.\n"
    "- If a new implementation is still necessary, keep single-use logic local; "
    "extract a shared abstraction only when concrete reuse justifies it.\n"
    "\n"
